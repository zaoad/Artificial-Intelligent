...
int dist[100], parent[100], huristic[100];
int n;
...
void astar(int source,int goal)
{
    ...
    /////////////////////////////
    node s=node(source,0,0+huristic[source]);
    dist[source]=0;
    PQ.push(s);
    parent[source]=0;
    while(!PQ.empty())
    {
        ...
        for(int i=0;i<adj[u].size();i++)
        {
            ...
            if(dist[e.v]>dist[u]+e.w)
            {
                dist[e.v]=dist[u]+e.w;
                parent[e.v]=u;
                node b=node(e.v,dist[e.v],dist[e.v]+huristic[e.v]);
                PQ.push(b);
            }
        }
    }
...
int main()
{
    ...
    astar(chartoint(source_char),chartoint(destination_char));
